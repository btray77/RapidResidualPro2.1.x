...
if (ereg('VERIFIED',$output))
	...
	switch ($txn_type)
		...
		if($ttype == "inside")
			...
			$q = "select count(*) as cnt from ".$prefix."member_products where member_id='$mid' && type='$prodtype' && product_id='$pid'";
			$r = $db->get_a_line($q);
			$count	= $r[cnt];	
			if($count == '0')	
				{
	...
	if (eregi('recurring_payment_suspended_due_to_max_failed_payment',$txn_type))
		{	
		// Remove access to product	
    ...
    if (eregi('failed',$payment_status))
		...
		switch ($txn_type)
			...
			case 'subscr_payment':
			$q = "select count(*) as cnt from ".$prefix."orders where txnid='$subscr_id'";
			$r = $db->get_a_line($q);
			$count	= $r[cnt];
			if($count == '0')	
				{ 
			...
			break; // end subscr_payment
			} // end txn_type
		switch ($ttype)
    		{
			
			case 'outside':			
			$q = "select count(*) as cnt from ".$prefix."members where randomstring='$rand'";
			$r = $db->get_a_line($q);
			$c1	= $r[cnt];
			if($c1 == '0')	
				{ // enter member into database			
				...
				$q = "insert into ".$prefix."member_products set $set";
				$db->insert($q);								
				} // end enter member into database			
			break; // end outside			
		...
		$set    = "refunded='1' where txn_id='$parent_txn_id'";
		$q = "update ".$prefix."member_products set $set";
		$db->insert($q);
    	...
    	$set    = "payment_status='$payment_status' where txnid='$parent_txn_id'";
	    ...
	    switch ($txn_type)
			...
			$q = "select count(*) as cnt from ".$prefix."orders where txnid='$subscr_id' && payment_type='echeck'";
			$r = $db->get_a_line($q);
			$count	= $r[cnt];
			if($count == '0')	
				{ 
			...
			elseif($count != '0')
				{ // Pending order cleared
				// Clear pending from order
				$set    = "pending_reason='', payment_status='Completed' where txnid='$subscr_id'";
				$q = "update ".$prefix."orders set $set";
				$db->insert($q);
				} // end clear pending order				
...
if ($txn_type == 'subscr_cancel')
	...
	// Set order to refunded	
    $set    = "payment_status='Cancelled' where txnid='$subscr_id'";
	$q = "update ".$prefix."orders set $set";
	$db->insert($q);
	}
