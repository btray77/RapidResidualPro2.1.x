...
include "session.php";
include "header.php";
$GetFile = file("../html/admin/arp.html");
$Content = join("",$GetFile);
$Title = "Autoresponse Plus Management";
...
$q = "select count(*) as cnt from ".$prefix."responders where rspname ='ARP'";
$r = $db->get_a_line($q);
$count = $r[cnt];
$records=10;
$links = "arp.php?";
...
$GetProducts = $db->get_rsltset("select * from ".$prefix."responders where rspname ='ARP' order by id limit $start,$records");
$ChangeColor = 1;
$ToReplace = "";
...
$fieldNamesArray = array(
	...
	$query = "select count(*) as cnt from ".$tbl_name." where rspname ='ARP'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[cnt];

	/* Setup vars for query. */
	$targetpage = "arp.php"; 	//your file name  (the name of this file)
	if(isset ($_GET["limit"])){
		$limit = $_GET["limit"];
	}else{
	...
	$sql = "select * from $tbl_name where rspname ='ARP' order by $field $dir  limit $start,$limit";
...
$content = '
	 ...
	 if($obj_pri->canAdd($pageurl))
	 {
		 $content .= '<div class="buttons">
			<a href="addarp.php">Add New Responder</a>
		 </div>';
	 }
	$content .= '		
	 </td></tr>
...
while($row = mysql_fetch_array($result))
	...
	if($row['published']==0){
		...
		$content .='<td align="center" valign="middle" >'.$row['id'].'</td>
														...
														<a href="editarp.php?act=e&id='.$row['id'].'"><img src="../images/editIcon.png" alt="editImage" title="Click To Edit This Content Page" ></a> </div> 
