...
$GetFile = file("../html/admin/squeeze_view.html");
$Content = join("",$GetFile);

$Title = "Squeeze Page Management";
$targetpage = "squeeze_view.php"; 	//your file name  (the name of this file)
$tbl_name=$prefix."squeeze_pages";		//your table name
...
switch($act)
{
	...
	case 't':
		if($selectdir[0]=='0')
			header("location:$targetpage?msg=error");
		else 
		{
...
{
	case 'a':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze is successfully Added!</div>';
	break;
	case 'e':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze is successfully Edited!</div>';
	break;	
	case 'd':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze is successfully Deleted!</div>';
	break;
	case 't':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze Theme is successfully Modified!</div>';
	break;
	case 'ar':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze is successfully Unarchived!</div>';
	break;
	case 'un':
		$Message = '<div class="success"><img src="../images/tick.png" align="absmiddle"> Squeeze is successfully Archived!</div>';
	break;
	case 'error':
...
{
	$GetProd = $db->get_a_line("select * from ".$prefix."squeeze_pages where id = '$id'");
	@extract($GetProd);
	$filename = $filename;

	// If page has comments delete comments
	$db->insert("delete from ".$prefix."comments where page ='$filename' && type='squeeze'");
	$db->insert("delete from ".$prefix."squeeze_pages where id ='$id'");
...
function archive_content($id,$state,$db,$prefix)
{
	...
	$db->insert("update ".$prefix."squeeze_pages set published='$state' where id ='$id'");
	if($state==1)
		return $msg='ar';
	else 
		return $msg='un';
...
function update_theme($ids,$selectdir,$db,$prefix)
{
	...
	foreach($ids as $id)
	{
		echo $sql="update ".$prefix."squeeze_pages set asign_template='$selectdir[$i]' where id ='$id';";
		$db->insert($sql);
		$i++;
	}
		return $msg='t';
...
$fieldNamesArray = array(
	...
	function getContentComments($filename, $linkproduct, $prefix){
		...
		if($linkproduct == 'squeeze')
		{
			...
			$q = "select count(*) as cnt from ".$prefix."comments where type='squeeze' && page='$filename' && date
			between '$now_start' and '$now_end' && checked=0";
			
			$r22 = $db->get_a_line($q);
			$count1 = $r22[cnt];
			
			$q_total = "select count(*) as cnt from ".$prefix."comments where type='squeeze' && page='$filename'";
			$r_total = $db->get_a_line($q_total);
			$total = $r_total[cnt];
			
			 $q_unread = "select count(*) as cnt from ".$prefix."comments where type='squeeze' && page='$filename' && checked=0";
			...
			if($total > '0' && $count1 == '0' )
			{
				$commentlink='<a href="comment_moderation.php?pageid='.$filename.'&type=squeeze">
...
function select_Dir($dir_name,$pageid,$selected){
	...
	$content = '
	 ...
	 if($obj_pri->canAdd($pageurl))
	 {
		 $content .= '<div class="buttons">
		 	<a href="squeeze_add.php" >Add Squeeze Page</a>
		 </div>';
	 }
	$content .= '</td></tr> </table><form name="squeeze" action"" method="post"> ';
	...
	$content .= '<table id="table" border="0" cellpadding="5" cellspacing="0"
													...
													<thead>
														<tr class="list_results_colhead" id="pagination">
															...
															<th title="click to sort by this field" align="left" width="573">
															...
															<th width="140"><span>Access</span></th>
															<th width="140"><span>Choose Theme</span></th>
	...
	while($row = mysql_fetch_array($result))
	{
		...
		$select_dir = select_Dir($dir_name,$row['id'],$row['asign_template']);
	...
	if($obj_pri->canDelete($pageurl))
		{
			...
			<img src="../images/cross-gray.png" alt="editImage" title="Click to delete this squeeze page" Onclick="return confirm('."'".'Are you sure! you want to delete this page?'."'".')">
			</a>';
			}
			else
			{
			$delete='<a href="'.$targetpage.'?act=d&pageid='.$row['id'].'">
			<img src="../images/crose.png" alt="editImage" title="Click to delete this squeeze page" Onclick="return confirm('."'".'Are you sure! you want to delete this page?'."'".')">
			</a>';
			}
		}
		else{
			...
			if($row['published']==0){		
				$delete='<a href="'.$targetpage.'?act=a&pageid='.$row['id'].'&state=1">
					<img src="../images/cross-gray.png" alt="editImage" title="Click to unarchive this squeeze page">
			</a>';
				
			}
			else{
				$delete='<a href='.$targetpage.'?act=a&pageid='.$row['id'].'&state=0">
			<img src="../images/crose.png" alt="editImage" title="Click to archive this squeeze page">
			</a>';}
		...
		if($row['comments']=='no')
			...
			$comments=getContentComments($row['name'], 'squeeze', $prefix);
		...
		$content .='<td align="center" valign="middle" >'.$row['id'].'</td>
					<td valign="middle" width="300px">'.$row["name"].'</td>
					<td align="center" width="140">'.$row["access"].'</td>
					<td align="center" valign="middle" nowrap> ' . $select_dir . ' </td>
					<td align="center" valign="middle" nowrap>'. $comments .'</td>
					<td align="center" valign="middle">
						<a href="squeezelink.php?page='. $row['name'] .'&iframe=true&width=600px&height=150px" rel="prettyPhoto">
							<img src="../images/yellowlink.png" alt="linkImage" title="Click to go to the squeeze page">
						</a>
					</td>
					<td align="center" valign="middle">
						<div style="float:left;padding-left:16px;"><a href="squeeze_edit.php?act=e&id='.$row['id'].'">
							<img src="../images/editIcon.png" alt="editImage" title="Click to edit this squeeze page" ></a>
						 </div> 
