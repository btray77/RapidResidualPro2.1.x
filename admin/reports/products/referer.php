...
$targetpage = "referer.php?id=$id"; 	//your file name  (the name of this file)
...
$fieldNamesArray = array(
	...
	/*$query = "select count(m.id) as cnt from $tbl_name o ," . $prefix."members m
		WHERE m.username = o.referrer AND o.item_number =$id";*/
	
	$query = "select count(DISTINCT(o.referrer)) as cnt
	
	from $tbl_name o ," . $prefix."members m 
	WHERE m.username = o.referrer
	AND o.item_number =$id
	AND o.referrer <> ''
	AND o.referrer <> 'None'";
	
	$result = $db->get_a_line($query);
	$total_pages = $result['cnt'];
	
	//$total_pages = mysql_num_rows($rs_total);
...
//(SELECT COUNT(DISTINCT(referrer)) FROM ".$prefix."orders m  WHERE m.item_number = p.id and m.referrer <> '' and m.referrer <> 'None' ) AS refer,
    ...
    $sql = "select DISTINCT m.id as member_id,CONCAT(m.firstname,' ',m.lastname) as name,m.email,m.username, m.telephone,
	(select count(DISTINCT(referrer)) from ".$prefix."orders where referrer = m.username	) as buyer
	from $tbl_name o ," . $prefix."members m 
	WHERE m.username = o.referrer
	AND o.item_number =$id
	AND o.referrer <> ''
...
//	SELECT COUNT(id) FROM ".$prefix."orders m  WHERE m.item_number = p.id and m.referrer <> '' ) AS refer,		
...
<table class="notsortable" width="95%" border="0" align="center" cellpadding="2" cellspacing="0">
  ...
  <tr>
	...
	Contact#</a><span class="<?php echo getCssClass('field7',$dir,$fieldName);?>">&nbsp;</span></th>
	...
	Buyers</a><span class="<?php echo getCssClass('field6',$dir,$fieldName);?>">&nbsp;</span></th> -->
   </tr>
...
if($total_pages > 0)
{
 ...
 if ($i%2 == 0){ $class= "standardRow";} else{ $class= "alternateRow";}
  ...
  <tr class="<?php echo $class?>">
    ...
    <td valign="middle"><?php echo $row['member_id']?></td>
    ...
    <!-- <td valign="middle" style="text-align:center"> <?php echo $row['buyer']?></td> -->
  </tr>
...
echo "<tr><td colspan='6' style='text-align:center'>Sorry no record found</td></tr>";
 ?>
</table>
<div class="pages">
