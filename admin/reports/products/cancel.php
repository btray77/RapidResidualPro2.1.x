...
$Title = "Canceled Reports";

$targetpage = "refund.php?id=$id"; 	//your file name  (the name of this file)
...
$fieldNamesArray = array(
	...
	$query = "SELECT count(p.id) as cnt
				FROM  $tbl_name p ," . $prefix."members m, " . $prefix."orders o
				WHERE m.id=p.member_id and p.product_id=$id and p.txn_id = o.txnid and o.payment_status ='Cancelled'";
	...
	/* Get data. */
	 ...
	 $sql = "SELECT p.id,p.member_id,p.date_added, CONCAT(m.firstname,' ',m.lastname) as name,m.email,m.username 
				FROM  $tbl_name p ," . $prefix."members m, " . $prefix."orders o
				WHERE m.id=p.member_id and p.product_id=$id and p.txn_id = o.txnid and o.payment_status ='Cancelled'  
				ORDER BY $field $dir  
				LIMIT $start,$limit";
...
<table class="notsortable" width="95%" border="0" align="center" cellpadding="2" cellspacing="0">
  ...
  <tr>
	...
	Order Date</a><span class="<?php echo getCssClass('field6',$dir,$fieldName);?>">&nbsp;</span></th>
   </tr>
