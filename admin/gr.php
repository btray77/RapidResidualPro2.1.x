...
include "session.php";
include "header.php";
$GetFile = file("../html/admin/gr.html");
$Content = join("",$GetFile);
$Title = "Get Response Management";
...
$q = "select count(*) as cnt from ".$prefix."responders where rspname ='GetResponse'";
$r = $db->get_a_line($q);
$count = $r[cnt];
$records=10;
$links="gr.php?";
...
/*$GetProducts = $db->get_rsltset("select * from ".$prefix."responders where rspname ='GetResponse' order by id limit $start,$records");
$ChangeColor = 1;
$ToReplace = "";
...
/*if($msg == "add")
{
	$Message = "Responder is Successfully Added";
}
else if($msg == "edit")
{
	$Message = "Responder is Successfully Edited";
}
else if($msg == "d")
{
	$Message = "Responder is Successfully Deleted";
}
...
$fieldNamesArray = array(
	...
	$query = "select count(*) as cnt from ".$tbl_name." where rspname ='GetResponse'";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[cnt];

	/* Setup vars for query. */
	$targetpage = "gr.php"; 	//your file name  (the name of this file)
	if(isset ($_GET["limit"])){
		$limit = $_GET["limit"];
	}else{
	...
	$sql = "select * from $tbl_name where rspname ='GetResponse' order by $field $dir  limit $start,$limit";
...
$content = '
	 ...
	 if($obj_pri->canAdd($pageurl))
	 {
		 $content .= '<div class="buttons">
			<a href="addgr.php">Add New Responder</a>
		 </div>';
	 }
	$content .= '		
	 </td></tr>
...
while($row = mysql_fetch_array($result))
	...
	if($row['published']==0){
		...
		$content .='<td align="center" valign="middle" >'.$row['id'].'</td>
														...
														<a href="editgr.php?act=e&id='.$row['id'].'"><img src="../images/editIcon.png" alt="editImage" title="Click To Edit This Content Page" ></a> </div> 
...
/*$q = "select count(*) as cnt from ".$prefix."responders where rspname ='Aweber'";
$r = $db->get_a_line($q);
$count = $r[cnt];
$records=10;
$links="aweber.php?";
